...
const CONCEPT_BUCKETS = [
  { name: "AI & ML",                  re: /machine learning|deep learning|neural network|computer vision|\bcnn\b|\bnlp\b|reinforcement learning|model (?:training|inference)|\binference\b|object detection|image (?:classification|recognition|segmentation)|classifier|pose estimation|gesture recognition|transformer|\bllms?\b|large language model|generative ai|\bgpt\b|openai|anthropic|\bclaude\b|gemini|langchain|llama|\brag\b|retrieval.?augmented|prompt engineering|embeddings?|vector (?:db|database|store|search)|chroma|faiss|pinecone|weaviate|hugging.?face|fine.?tun|agentic|\bai agent|policy[ -]gradient|reward[ -](?:model|hack)|\bgae\b|\bkl\b control|rerank|pagerank|nearest neighbor|grounded generation|react agent|agent (?:loop|workflow)|self-correction|set-of-marks|structured output|typed action space|hallucination|hierarchical classification|probabilistic reasoning|accessibility-tree/i },
  { name: "Algorithms & DS",          re: /algorithm|data structure|\bgraph\b|breadth-first|depth-first|\bsearch|\bsort|recursion|combinator|state.?space|traversal|dynamic programming|minimax|alpha-?beta|pathfind|\ba\*|complexity|\boptimization|\bsolver\b|tree & set|string processing|computational geometry|competitive programming/i },
  { name: "Statistics & Evaluation",  re: /statistic|significance test|hypothesis test|confidence interval|\bwilson\b|\bp-value\b|\bablation\b|evaluation (?:harness|methodolog)|benchmark (?:harness|scoring)|\bpass@k\b|reproducib|\ba\/b test|cross-evaluator|evaluator agreement/i },
  ...
  { name: "Systems & Embedded",       re: /robotics|embedded|firmware|microcontroller|arduino|raspberry pi|\brtos\b|\bros\b|sensor fusion|actuator|bare-metal|control system|device driver|\bkernel\b|\bsimd\b|\bsensor\b|motor control|serial communication|hardware calibrat|bluetooth/i },
  ...
  { name: "Architecture & Design",    re: /object-?oriented|\boop\b|design pattern|\bpattern\b|architecture|\bmvvm\b|\bmvc\b|modular|encapsulat|abstraction|repository|observer|factory|singleton|state management|lifecycle|persistence|data model|\bnavigation\b|dependency injection|inheritance|polymorph|software design|separation of concerns|event-driven|component reuse|module system|strategy-style/i },
  { name: "Testing & Delivery",       re: /\btest|debug|invariant|ci.?\/?cd|coverage|packaging|deploy|shipping|build system|cross-platform|devops|benchmark|build and ci/i },
  ...
  { name: "Data & Visualization",     re: /data analysis|data visuali|interactive.*visuali|image (?:processing|scaling)|grayscale|\bpillow\b|pdf parsing|\bplot/i },
  { name: "Reliability & Ops",        re: /error handling|fault tolerance|resilien|crash-safe|secrets management|dependency management|cost (?:engineering|\/?token)|privacy engineering|checkpoint|incremental computation|failure taxonomy|performance optimi|accessibility and performance/i },
  { name: "Platform & I/O",           re: /local notification|preferences storage|keyboard shortcut|layout management|input handling|audio playback|file i\/o|clipboard|mobile development|driver management/i },
];

  ...
  return (
    <header className="mx-auto flex min-h-[88svh] max-w-[1080px] flex-col justify-center gap-12 px-6 py-20 md:flex-row md:items-center md:gap-16">
      ...
      <div ref={identityTilt} className="flex-1">
        <p data-depth="28" className="font-mono text-[12px] uppercase tracking-[0.32em] text-pine">
          <ScrambleText text={content.hero.eyebrow} delay={0} />
        </p>
        <h1 data-depth="60" className="mt-4 font-display text-[clamp(44px,8vw,72px)] font-semibold leading-[1.04] tracking-tight text-walnut">
          <ScrambleText text={PERSON.name} delay={120} />
        </h1>
        <p data-depth="24" className="mt-4 max-w-[42ch] font-sans text-[18px] leading-relaxed dark:leading-[1.72] text-walnut/75">
          <ScrambleText text={content.hero.tagline} delay={300} />
        </p>
        {currentStatus && (
          <p data-depth="16" className="mt-2 font-mono text-[13px] text-walnut/55">
            <ScrambleText text={`${content.hero.currentlyLabel} ${currentStatus}`} delay={480} />
          </p>
        )}

        <nav data-depth="14" className="mt-7 flex flex-wrap items-center gap-x-5 gap-y-2 font-mono text-[13px]">
          <PressButton
            onClick={onResume}

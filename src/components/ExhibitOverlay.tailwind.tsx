        ...
        <>
          ...
          >
              <motion.div
                ...
                style={{ maxHeight: "min(460px, 72vh)" }}
              >
                ...
                {/* Body — takes remaining height, each section scrolls independently */}
                <div className="flex-1 min-h-0 flex flex-col bg-parchment overflow-hidden">

                  {/* Description + tags sidebar — both scroll independently */}
                        ...
                        <div className="shrink-0 w-40 flex flex-col gap-1.5 px-4 py-4 border-l border-[rgba(58,46,30,0.1)] overflow-y-auto [scrollbar-width:thin] [scrollbar-color:#7a9e7e_transparent]">
                          ...
                          {popup.skills?.flatMap((group, i) => {
                            ...
                            return group.items.map((item) => (
                              <span
                                key={item}
                                className="font-mono text-[11px] text-walnut rounded px-2 py-0.5 border"
                  ...
                  {popup.embedUrl && (
                    <iframe
                      src={popup.embedUrl}
                      className="flex-1 min-h-0 border-0 bg-black"
                      style={{ height: `min(${popup.height ?? "650px"}, 70vh)` }}
                      allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope"
                    />
                  )}

                  {/* Links — fixed at bottom, never scrolls */}
